/* * Last edited: Aug 20 20:34 1992 (trapp) */
/* 
...
# define gc_malloc saint_calloc
# define gc_init saint_calloc_initialize
...
struct gc_obj * gc_malloc_atomic(lb)
int lb;
{
register struct gc_obj *op;
register struct gc_obj **opp;
...
#       endif
	opp = &(aobjfreelist[lw]);
        if( (op = *opp) == ((struct gc_obj *)0) ) {
	    op = _allocaobj(lw);
        }
#       ifdef DEBUG
	    if ((op -> obj_link != ((struct gc_obj *) 0)
...
int is_atomic;
	...
	if (lb >= (sz >> 1)) {
	    ...
	    /* shrink */
	      struct gc_obj * result = gc_malloc_atomic(lb);
	...
	if (lb >= (sz >> 1)) {
	    ...
	    /* shrink */
	      struct gc_obj * result = gc_malloc(lb);
...
/* Added for SCO Unix, Michael Arnoldus (chime@diku.dk), 8 Mar 1992 */
#ifdef SCO
