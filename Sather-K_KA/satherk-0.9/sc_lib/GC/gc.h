/* * Last edited: Mar 17 14:28 1992 (hws) */
/*
 * Merged Michael Arnoldus' patches for SysV SCO 3.2.
 * struct obj is renamed to gc_obj, because there is obj in standard Sys V.
 *
...
#ifdef SCO
/* Due to sys V lack of bcopy */
#define bcopy(src,dst,sz)	memcpy(dst,src,sz)
...
#	    ifdef RISCOS
#             ifdef SYSTYPE_BSD43
#               define STACKTOP ((word *) 0x7ff80000)
#             else
#               ifdef sony_news
#                 include <sys/param.h>
#                 include <sys/vmparam.h>
#                 define STACKTOP USRSTACK
#               else
...
#             else
#		ifdef SCO
#		  define STACKTOP ((word *) 0x7ffffffc) /* For SCO */
#		else
...
#             else
#	        ifdef SCO
#		  define DATASTART ((char *)(0x00400000+((long)&etext & 0xFFC00000)+((long)&etext & 0xFFC00FFF)))
#               else
...
# define HINCR_MULT 2      /* After each new allocation, hincr is multiplied */
# define HINCR_DIV 1       /* by HINCR_MULT/HINCR_DIV                        */
...
struct __gc_arrays {
  struct gc_obj * _aobjfreelist[MAXAOBJSZ+1];
			  /* free list for atomic objs*/
  struct gc_obj * _objfreelist[MAXOBJSZ+1];
...
struct gc_obj {
    union {
	struct gc_obj *oun_link;   /* --> next object in freelist */
#         define obj_link       obj_un.oun_link
	word oun_component[1];  /* treats gc_obj as list of words */
...
extern struct gc_obj * allocobj();
extern struct gc_obj * allocaobj();

/* Small object allocation routines that mark only from registers */
/* expected to be preserved by C.                                 */
extern struct gc_obj * _allocobj();
extern struct gc_obj * _allocaobj();

/* general purpose allocation routines */
extern struct gc_obj * gc_malloc();
extern struct gc_obj * gc_malloc_atomic();
